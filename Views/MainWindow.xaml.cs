    ...
    public partial class MainWindow : Window
    {
        ...
        private void AcceptRequest_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as Button)?.CommandParameter is IncomingRequest request)
            {
                ...
                else if (request.Type == RequestType.File)
                {
                    ...
                    HandleIncomingFile(args.Client, args.Stream, args.FileName, args.FileSize, args.RemoteUser, accepted: true);
                }
            }
        }
        ...
        private void RejectRequest_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as Button)?.CommandParameter is IncomingRequest request)
            {
                ...
                {
                    var args = (FileRequestEventArgs)request.EventArgs;
                    // This method now handles the logging and rejection network message
                    HandleIncomingFile(args.Client, args.Stream, args.FileName, args.FileSize, args.RemoteUser, accepted: false);
                }
            }
        }
        #endregion
        ...
        private async void SendFile_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                await manager.SendFileAsync(
                    _selectedFilePath, _currentUser, _selectedPeer.IP, _selectedPeer.Port, _selectedPeer.Username,
        ...
        private void HandleIncomingFile(TcpClient client, NetworkStream stream, string fileName, long encryptedFileSize, string remoteUser, bool accepted)
        {
            if (!accepted)
            {
                Task.Run(async () =>
                {
                    // The owner of a "Rejected" log is the current user (the one who clicked "Reject").
                    await LogService.LogFileTransferAsync(fileName, 0, remoteUser, _currentUser, "Rejected", _currentUser);
            ...
            {
                // User cancelled the save dialog, which is the same as rejecting.
                Task.Run(async () =>
                {
                    await LogService.LogFileTransferAsync(fileName, 0, remoteUser, _currentUser, "Rejected", _currentUser);
                    await WriteChunkAsync(stream, Encoding.UTF8.GetBytes("REJECT"));
            ...
            Task.Run(async () =>
            {
                try
                {
                    await manager.ReceiveFileAsync(
                        fileName, stream, encryptedFileSize, savePath, remoteUser, _currentUser,
                        _rsaPrivateKey, _rsaModulus, progress, _transferCts.Token);
        ...
        private void OnFileRequestReceived(object? sender, FileRequestEventArgs e)
        {
            ...
            var request = new IncomingRequest { Type = RequestType.File, Message = $"File '{e.FileName}' ({fileSizeKB} KB) from {e.RemoteUser}", EventArgs = e };
            Dispatcher.Invoke(() => _incomingRequests.Add(request));
        }
        #endregion
        ...
        {
            var logWindow = new FileLogWindow(_currentUser);
            logWindow.Show();
        }
